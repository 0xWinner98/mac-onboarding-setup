...
| 飞书 CLI | 让 AI 读写飞书表格 / 文档 | 官方 npx 安装器（CLI 本体 + Agent Skills） |
...
**Q：某个工具安装失败？**
多半是网络。把那几行红色 ❌ 截图发到群里。可以重跑脚本，已装好的会自动跳过。
...
- 中转走 CC Switch（学员自己装 + 学员自己去中转站注册拿 key），脚本只负责"带你装上 CC Switch"。
...
- 飞书 CLI：`npx --yes @larksuite/cli@latest install`（官方安装器：CLI 本体 + Agent Skills）
